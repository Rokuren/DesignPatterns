 ...
 *  IE, letters in a document editor. Each characters(intrinsic) are their own object.
 *    However, other qualities are extrinsic(row,column,font) 
 *  FlyweightFactory: creates and manages flyweight objects;
 *    ensures that flyweights are shared properly. Each flyweight is a singleton.
 *  Flyweight:(DialogBox)
 *    declares an interface through which flyweights can receive and act
 *    on an extrinsic state.
 *  ConcreteFlyweight: (CommitTransaction,FileSelection)
 *    implements implements flyweight interface.
 *  Client: (main)
...
int FlyweightFactor::_numIcons = 0;
